...
| `esp32-core/` (**standalone-appliance third core**) — the ESP32-S3 radio core: ESP-IDF + NimBLE, the clean-room **C MouldKingCrypt port**, the MK4 advertiser (`0xFFF0`), the **300 ms auto-neutral safety layer**, the WiFi **WS server mirroring `api.py`**, **WiFi provisioning** (no creds baked in, fallback `moldqueen-setup` AP), **mDNS** `moldqueenesp.local`, and a **management page** (`:8080`); Pi mDNS (`moldqueenrasp.local` for linux-core) + the binary/release pipeline (`esp-v*` tag → CI builds one flashable `.bin` → GitHub Release) shipped; scope complete — serve-client-from-flash considered and decided against (radio core only) | **esp32-core-dev** |
