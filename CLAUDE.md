...
| `esp32-core/` (**working third core**) — the ESP32-S3 radio core: ESP-IDF + NimBLE, the clean-room **C MouldKingCrypt port**, the MK4 advertiser (`0xFFF0`), the **300 ms auto-neutral safety layer**, and the WiFi **WS server mirroring `api.py`** (drives real toys over WiFi); remaining: provisioning + serving the client from flash | **esp32-core-dev** |
