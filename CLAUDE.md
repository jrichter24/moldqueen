...
| `esp32-core/` (**standalone-appliance third core**) — the ESP32-S3 radio core: ESP-IDF + NimBLE, the clean-room **C MouldKingCrypt port**, the MK4 advertiser (`0xFFF0`), the **300 ms auto-neutral safety layer**, the WiFi **WS server mirroring `api.py`**, **WiFi provisioning** (no creds baked in, fallback `moldqueen-setup` AP), **mDNS** `moldqueenesp.local`, and a **management page** (`:8080`); next: Pi mDNS (`moldqueenrasp.local` for linux-core), then the binary/release pipeline, serve-client-from-flash after | **esp32-core-dev** |
