...
(function () {
  ...
  var DE = {
    ...
    "ix.tiles.note": "Die Karten kommen live aus dem Projekt-Manifest. Die Abzeichen sind dieselben wie in der App: <strong>Generic</strong> oder <strong>Model</strong>, dazu das unterstützte Protokoll. <strong>MK4</strong> und <strong>MK6</strong> sind jetzt beide aktiv (die generischen Layouts tragen beide); MK6 läuft auf dem Raspberry Pi und dem Web-Client. Mit der Maus über eine Karte fahren, dann klappt sie auf.",
    // ---- index: get the app ----
    ...
    "ix.and.proto": "Spricht <b>MK4</b>. MK6 und der Mischbetrieb aus MK4+MK6 laufen vorerst auf dem Raspberry-Pi-Kern.",
    ...
    "ix.pi.proto": "Spricht sowohl <b>MK4</b> als auch <b>MK6</b> und kann eine MK4- und eine MK6-Box gleichzeitig fahren. Der einzige Kern, der das kann.",
    ...
    "ix.esp.proto": "Spricht <b>MK4</b>. MK6 und der Mischbetrieb aus MK4+MK6 laufen vorerst auf dem Raspberry-Pi-Kern.",
    // ---- index: capability capstone (server/client matrix) ----
    "ix.combine.eyebrow": "Frei kombinieren",
    "ix.combine.h3": "Ein Client, jeder Funk.",
    "ix.combine.lead": "Es gibt einen smarten Client, und er läuft überall gleich: als Seite vom Pi, in der Android-WebView, in einem Docker-Container, auf dem Desktop-Dev-Server und bald auf iPhone und Mac. Lern die Oberfläche einmal und kombiniere jeden Server mit jedem Client. Die Tabelle zeigt, was jeder Build heute mitbringt.",
    "ix.combine.col0": "Build",
    "ix.combine.colServer": "Server (Funk)",
    "ix.combine.colClient": "Client (UI)",
    "ix.combine.colProto": "Funk-Protokolle",
    "ix.combine.protoPi": "MK4 + MK6 + gemischt",
    "ix.combine.rAndroid": "Android-App",
    "ix.combine.rPi": "Raspberry Pi <a class=\"cap-fn\" href=\"#combine-note\" aria-describedby=\"combine-note\"><sup>1</sup></a>",
    "ix.combine.rEsp": "ESP32",
    "ix.combine.rDesktop": "Desktop-Dev-Client",
    "ix.combine.rDocker": "Docker-Image",
    "ix.combine.rAndroidS": "Android (nur Server)",
    "ix.combine.rAndroidC": "Android (nur Client)",
    "ix.combine.rApple": "iPhone / Mac Client",
    "ix.combine.planned": "geplant",
    "ix.combine.note": "<sup>1</sup> Der Pi ist ein Monolith: Er kann Server und Client zusammen laufen lassen oder gezielt nur als Server oder nur als Client.",
    // ---- index: docker client ----
    "ix.dock.eyebrow": "Docker-Client",
    "ix.dock.h2": "Den Client mit einem Befehl starten.",
    "ix.dock.lead": "Du hast schon einen Funkkern im Netz? Dann spar dir den Build komplett. Ein veröffentlichter, öffentlicher Container hostet allein die Web-Oberfläche, sodass ein einziger Befehl den Client auf deinen Rechner bringt und du ihn auf dein Gerät richtest.",
    ...
    "ix.dev.k4": "Docker",
    "ix.dev.v4": "Ein veröffentlichtes, öffentliches Nur-Client-Image, ein Befehl, kein Build. Siehe den Abschnitt <a href=\"#docker\">Docker-Client</a>. <a href=\"https://github.com/jrichter24/moldqueen/blob/main/dev-docs/REMOTE_CLIENT.md\" rel=\"noopener\" target=\"_blank\">REMOTE_CLIENT.md</a>",
    "ix.dev.callout": "<strong>Das Protokoll.</strong> Gesteuert wird über herstellereigene Bluetooth-LE-Advertisements: ein Telegramm aus zwölf Nibble-Kanälen (MK4), das alle verbundenen Hubs gleichzeitig ansteuert. Auf dem Raspberry Pi und dem Web-Client kommt außerdem MK6 hinzu (ein byteweises, gerätadressiertes Telegramm), und beide können eine MK4- und eine MK6-Box gleichzeitig fahren; die ESP32- und Android-Kerne sprechen vorerst nur MK4. Der Codec ist Byte für Byte gegen die offizielle App geprüft. Vollständige Referenz in <a href=\"https://github.com/jrichter24/moldqueen/blob/main/dev-docs/PROJECT.md\" rel=\"noopener\" target=\"_blank\">PROJECT.md</a> und im maschinenlesbaren <a href=\"https://github.com/jrichter24/moldqueen/blob/main/linux-core/mk4web/asyncapi.yaml\" rel=\"noopener\" target=\"_blank\">AsyncAPI</a>.",
    ...
    "ix.rm.mk6": "<b>MK6 auf dem ESP32- &amp; Android-Kern.</b> <span>MK6 (Mould Kings byteweises Protokoll) und der gleichzeitige Mischbetrieb aus MK4 und MK6 laufen bereits auf dem Raspberry Pi und dem Web-Client, weshalb die generischen Layouts ein aktives MK6-Abzeichen tragen. Offen ist noch, den MK6-Telegrammpfad auf die ESP32- und Android-Funkkerne zu bringen. <a href=\"https://github.com/jrichter24/moldqueen/blob/main/dev-docs/MIXED_MODE.md\" rel=\"noopener\" target=\"_blank\">Wie der Mischbetrieb funktioniert</a>.</span>",
    "ix.rm.esp": "<b>ESP32-Kern: fertig.</b> <span>Der ESP32 ist ein vollständiger eigenständiger Funk-Kern: WLAN-Einrichtung am Board, Auffinden als <code>moldqueenesp.local</code>, eine Verwaltungsseite, mDNS für den Pi (<code>moldqueenrasp.local</code> für den Raspberry-Pi-Kern) und eine Binary-/Release-Pipeline (eine herunterladbare <code>.bin</code>) sind alle vorhanden. Er bleibt ein schlanker Funk-Kern, den ein gehosteter Client steuert; das Ausliefern des Clients aus dem eigenen Flash wurde erwogen und verworfen.</span>",
