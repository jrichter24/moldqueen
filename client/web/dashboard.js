...
// The chevron points the way the menu MOVES (mirrors shell.css's sidebar breakpoint):
// the collapse control points toward where the menu hides, the expand chip toward where it
// returns. TOP BAR → collapse "▴" (up) / expand "▾" (down); SIDEBAR → collapse "‹" (left) / expand "›" (right).
const SIDEBAR_MQ = "(max-width: 768px), (max-height: 540px)";
function isSidebar() { return window.matchMedia(SIDEBAR_MQ).matches; }
function collapseGlyph() { return isSidebar() ? "‹" : "▴"; }   // toward where the menu collapses: sidebar→left, top bar→up
function expandGlyph() { return isSidebar() ? "›" : "▾"; }     // toward where the menu expands: sidebar→right, top bar→down
